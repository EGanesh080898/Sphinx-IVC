...
//NOTE: Below hash defined taken from IvcPubSubTopicList.hpp
#define MAX_TOPIC_LIST_SIZE  229
#define MAX_TOPIC_NAME_SIZE  32
#define TOPIC_LIST_NAME      "topiclist"

typedef std::array<std::array<char, MAX_TOPIC_NAME_SIZE>, MAX_TOPIC_LIST_SIZE> topicData;
...
err:
    IVC_LOG_DEBUG("Number of shared memories detected = %d\n", memCnt);
...
bool shmem_getSharedMemory(std::string& memName, uint32_t& idx, void **memAddr, uint32_t& memSize)
{
...
bool shmem_get_topicList(std::string memName, std::vector<std::string>& topiclist)
{
    void     *memAddr;
    uint32_t  idx;
    uint32_t  shmemSize = 0;
    ...
    if(true == shmem_getSharedMemory(memName, idx, &memAddr, shmemSize))
    {
        void *readPtr = NULL;
        readPtr = ((int8_t *)memAddr) + sizeof(lgivc_shmem_hal_meta);
        //IVC_LOG_INFO("%s is mapped @  %p with size: %ld using device: uio%d", memName.c_str(), memAddr, shmemSize, idx);
        std::cout << memName << " is using device: uio" << idx << " having shared memory size: " << formatBytes(shmemSize) << std::endl;
        memcpy(reinterpret_cast<void *>(&topicArray), readPtr, sizeof(topicArray));
        for(auto ta : topicArray)
        {
            if(strlen(ta.data()))
            {
                topiclist.push_back(ta.data());
                ret = true;
            }
            else
            {
...
bool shmem_getTopicMapping(std::vector<std::string>& topicList)
{
    void     *memAddr;
    uint32_t  idx;
    uint32_t  shmemSize = 0;
...
int main()
{
    ...
    std::string topiclist = TOPIC_LIST_NAME;

    setIvcLogLevel(IVC_LOG_INFO);

    if(shmem_get_topicList(topiclist, topicList) == true)
    {
        shmem_getTopicMapping(topicList);
    }
